...
def build_H_nearest_neighbour(system: QuantumSystem) -> np.array:

    N = system.sites
    # Change into atomic units if appropriate
    hbar = 1 if system.atomic_units else constants.hbar
    ...
    if system.interaction_model == 'nearest_neighbour_cyclic':
        H[0][N-1] = 1
        H[N-1][0] = 1
